...
<?php
	if(@$ac=='editar-rm') {
		$sql = mysql_query("update comb_rm set km_inicial = '$km_inicial', data = '$data', data_ref = '$data_ref', equipe = '$equipe' WHERE id = '$id'");
		if($sql) {
	...
	if(@$ac=='listar') {
		...
		if(@$op=='del') { mysql_query("delete from comb_rm_itens where id = $item"); }

		

		echo '<table class="table table-bordered table-condensed small table-success">';

		echo '<thead><tr><th>Data</TH><th>Cupom</th><th>Qtd</th><th>Valor</th><th>Total</th><th>Odômetro</th><th class="text-center">Obs</th><th>Funcionario</th><th class="text-center"><span class="glyphicon glyphicon-edit"></span> </th><th class="text-center"><span class="glyphicon glyphicon-trash"></span> </th></tr></thead><tbody>';
...
<form action="javascript:void(0)" class="formulario-blue" onsubmit="post(this,'financeiro/editar-comb-rm-novo.php?ac=editar-rm&id=<?php echo $cod_rm ?>','.resultado')">

<table class="table table-condensed table-bordered table-color">
	<thead>
	<tr class="active"><th>Placa:</th><th>KM Inicial:</th><th>Data:</th><th></th></tr>
	</thead>
	<tbody>
	<tr>
		...
		while($li = mysql_fetch_array($detalhes_rm)) {
			...
			echo '<td><input type="text" name="km_inicial" value="'.$li['km_inicial'].'" class="form-control"></td>';
...
<form action="javascript:void(0)" class="formulario-blue" onSubmit="post(this,'financeiro/editar-comb-rm-novo.php?ac=listar&op=inserir&cod_rm=<?php echo $cod_rm ?>','.ajax')">

	<table class="table table-bordered table-condensed table-striped table-blue small">
	<thead>
			...
			<label style="width:100%">
				<select name="funcionario" class="form-control input-sm combobox" required>
					...
					<?php
						...
						$fun = mysql_query("SELECT * FROM rh_funcionarios WHERE (obra IN($obu) OR tipo_emp = '1') AND situacao NOT IN(4,9,1,5,15,16,18) ORDER BY nome");
...
<script>ldy("financeiro/editar-comb-rm-novo.php?ac=listar&cod_rm=<?php echo $cod_rm ?>",".ajax")</script>
