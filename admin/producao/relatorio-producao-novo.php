...
<?php if(@$ac == 'filtrar') {
      ...
      if(mysql_num_rows($query) > 0) {
         ...
         if($cav==1) {
         $total_quantidade = @mysql_result(mysql_query("select *, SUM(qtd) as total_quantidade from ss_se, ss_itens where equipe = '$equipe' and (data between '$inicial_en' and '$final_en') and ss_se.servico = ss_itens.id and ss_itens.producao <> 0"),0,"total_quantidade"); }
         else {$total_quantidade = @mysql_result(mysql_query("select *, SUM(qtd) as total_quantidade from ss_se, ss_itens where equipe = '$equipe' and (data between '$inicial_en' and '$final_en') and ss_se.servico = ss_itens.id and ss_itens.producao <> 0 and ss_itens.producao <> 10"),0,"total_quantidade"); } 
         
         //$cav = @mysql_result(mysql_query("select * from equipes where id = $equipe"),0,"cav");
		 //if($cav=='0' AND $id_producao == 10) { $total_producao = 0; } else { $total_producao = $total_producao; }
         ...
         if($saldo_quantidade < 0) { $saldo_qtd = '<font color="#FF0000"><b>'.$saldo_quantidade.'</b></font>'; } else { $saldo_qtd = '<font color="#000">'.$saldo_quantidade.'</font>'; }
              ...
              echo '<td style="width:100px;" align="center">'.$saldo_qtd.'</td>';
                ...
                $total_v = 0;
                if($cav==1) {
                $servs = mysql_query("select *, sum(qtd) as total from ss_se where equipe = '$equipe' and (data between '$inicial_en' and '$final_en') group by servico"); }
                else { $servs = mysql_query("select *, sum(qtd) as total from ss_se, ss_itens where equipe = '$equipe' and (data between '$inicial_en' and '$final_en') and ss_se.servico = ss_itens.id and ss_itens.producao <> 10 group by servico"); }
                while($li = mysql_fetch_array($servs)) {

                  $serv = $li['servico'];
                  $producao_servico = mysql_result(mysql_query("select * from ss_itens where id = $serv"),0,"producao");
                  $valor_u = @mysql_result(mysql_query("select * from sp where id = $producao_servico"),0,"valor");
                  $total_g = $li['total']*$valor_u;
                  $total_v += $total_g;
...
<form action="javascript:void(0)" onSubmit="post('#form1','producao/relatorio-producao-novo.php?ac=filtrar','.retorno')" id="form1">
