<?php include("../../config.php");
	...
	if(@$ac=='excluir') { 
		$query = mysql_query("delete from notas_itens where id = '$id'"); 
		if($query) { 
			...
			echo '<p class="text-danger">Item não pode ser excluido</p>'; 
			echo '<p class="text-danger">'.mysql_error().'</p>'; 
		}
		exit;
	}
	
	$equipamento = mysql_query("SELECT * FROM notas_itens WHERE id = '$id'") or die (mysql_error());
	...
	echo '
		<div class="ajax">
			<center>
			<a href="javascript:void(0)" class="btn btn-success btn-sm" style="width:150px; margin-right:20px;" onclick=\'ldy("almoxarifado/del/excluir-mat-polemica.php?ac=excluir&id='.$id.'",".ajax")\'>Sim</a>
