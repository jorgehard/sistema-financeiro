<?php include("../../config.php");
	...
	if(@$ac=='excluir') { 
		$query = mysql_query("delete from encarregados where id = '$id'"); 
		if($query) { 
	...
	$equipamento = mysql_query("SELECT * FROM encarregados WHERE id = '$id'") or die (mysql_error());
	if( mysql_num_rows($equipamento) == 0 )
	{
		echo '
			<center>
				<div class="alert alert-danger" style="font-size:12px">
					Encarregado não encontrada!!! Ou ja excluida
 				</div>
			</center>';
		echo '
			<div class="ajax">
	...
	echo '
		<center>
			<div class="alert alert-danger" style="font-size:12px">
				Tem certeza que deseja excluir este encarregado <strong>'.$nome.'</strong> permanentemente?
			</div>
		</center>';
	echo '
		<div class="ajax">
			<center>
			<a href="javascript:void(0)" class="btn btn-success btn-sm" style="width:150px; margin-right:20px;" onclick=\'ldy("gestor/del/ex-enca.php?ac=excluir&id='.$id.'",".ajax")\'>Sim</a>
