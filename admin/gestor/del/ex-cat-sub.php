<?php include("../../config.php");
	...
	if(@$ac=='excluir') { 
		$query = mysql_query("delete from notas_cat_sub where id = '$id'"); 
		if($query) { 
	...
	$equipamento = mysql_query("SELECT * FROM notas_cat_sub WHERE id = '$id'") or die (mysql_error());
	...
	echo '
		<center>
			<div class="alert alert-danger" style="font-size:12px">
				Tem certeza que deseja excluir a sub-categoria <strong>'.$nome.'</strong> permanentemente?
			</div>
		</center>';
	echo '
		<div class="ajax">
			<center>
			<a href="javascript:void(0)" class="btn btn-success btn-sm" style="width:150px; margin-right:20px;" onclick=\'ldy("gestor/del/ex-cat-sub.php?ac=excluir&id='.$id.'",".ajax")\'>Sim</a>
